...
SOURCES = [
    "src/util.asm",
    "src/check_victory.asm",
    "src/buffer.asm",
    "src/merge.asm",
    "src/move_check.asm",
    "src/move_left.asm",
    "src/complete_move.asm",
    "src/move_one.asm",
    "src/place.asm",
    "src/printboard.asm",
    "src/points.asm",
    "tests/test_utils.asm"]
...
def run_test(test, timeout):
    ...
    try:
        ...
        subprocess.run(["java", "-jar", "mars-4.3-SNAPSHOT.jar", f"ae{AE_CODE}", f"se{SE_CODE}", "me", "nc", "sm", str(MAX_INSTRUCTIONS), *SOURCES, test + ".asm"],
